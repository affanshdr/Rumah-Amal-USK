        ...
        <div className="p-5 sm:p-6 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="bg-[#063A1E] text-white rounded-xl p-4 flex items-center justify-between gap-3 shadow-xs">
            <div>
              ...
              <p className="font-bold text-base capitalize">{item.jenisInfaq}</p>
            </div>
            <div className="text-right">
              <p className="text-[10px] font-semibold text-emerald-200 uppercase tracking-wider mb-0.5">Jumlah Infaq</p>
          ...
          <div className={`flex items-center gap-2 px-3.5 py-2.5 rounded-xl border text-xs font-semibold ${
            ...
            <FontAwesomeIcon
              ...
              className={`w-3.5 h-3.5 ${isTerikat ? 'text-emerald-600' : 'text-gray-400'}`}
            />
            {isTerikat ? (
              <span>Kampanye: <span className="font-bold">{item.kampanyeJudul}</span></span>
            ) : (
              <span>Infaq Bebas — tidak terikat program kampanye tertentu</span>
            )}
          </div>
